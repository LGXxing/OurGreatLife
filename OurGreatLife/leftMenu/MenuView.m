...
#define CoverViewBackGround [UIColor colorWithWhite:0 alpha:1.0]
...
@property (nonatomic, strong)UIScreenEdgePanGestureRecognizer *m_leftEdgeGesture;
@property (nonatomic, strong)UIView         *m_dependencyView;
@end


@implementation MenuView
@synthesize m_leftEdgeGesture;
...
-(void)addPanGestureAtDependencyView:(UIView *)dependencyView{
    ...
    // 屏幕边缘pan手势(优先级高于其他手势)
    m_leftEdgeGesture = [[UIScreenEdgePanGestureRecognizer alloc]initWithTarget:self action:@selector(handleLeftEdgeGesture:)];
    m_leftEdgeGesture.edges = UIRectEdgeLeft;// 屏幕左侧边缘响应
    m_leftEdgeGesture.delegate = self;
    self.m_dependencyView = dependencyView;
    [dependencyView addGestureRecognizer:m_leftEdgeGesture];
}


- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    if([gestureRecognizer isEqual:m_leftEdgeGesture]){
        CGPoint point = [touch locationInView:self.m_dependencyView];
        if(point.x < 10){
            return YES;
        }
        return NO;
